...
tape( 'the function throws an error if provided an invalid `normalized` option', function test( t ) {
	var values;
	var i;
...
tape( 'attached to the returned iterator is the generator seed', function test( t ) {
	...
	t.strictEqual( isInt32Array( it.seed ), true, 'has property' );
...
tape( 'attached to the returned iterator is the generator state', function test( t ) {
	var it = iterator();
	t.strictEqual( isInt32Array( it.state ), true, 'has property' );
	t.end();
});
...
tape( 'the function returns an iterator for generating pseudorandom integers on the interval `[1, 2147483646]`', function test( t ) {
	var rand;
	var it;
	var i;
...
tape( 'the function returns an iterator for generating pseudorandom numbers on the interval `[0,1)`', function test( t ) {
	var rand;
	var it;
	var i;
	...
	for ( i = 0; i < 1e3; i++ ) {
		t.strictEqual( rand.normalized(), it.next().value, 'returns expected value' );
	}
	t.end();
});
