...
function iterator( options ) {
	...
	if ( arguments.length > 0 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0q72V,FD', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'normalized' ) ) {
			if ( !isBoolean( opts.normalized ) ) {
				throw new TypeError( format( '0q72o,GE', 'normalized', opts.normalized ) );
			}
		}
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0q72t,G9', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
